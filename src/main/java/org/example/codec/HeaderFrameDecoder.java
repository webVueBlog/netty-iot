...
/**
 * HeaderFrameDecoder 用于解码头部帧
 */
@Component
@Scope(value = ConfigurableBeanFactory.SCOPE_PROTOTYPE)//每次请求都会创建一个新的实例
public class HeaderFrameDecoder extends ByteToMessageDecoder {//继承ByteToMessageDecoder，ByteToMessageDecoder是Netty提供的用于将字节流解码为消息的解码器
    ...
    private static final int LATITUDE_LENGTH = 8;// 纬度长度
    ...
    private static final int HEADER_FRAME_LENGTH =
            FRAME_HEADER_LENGTH + FRAME_TYPE_LENGTH + DATA_LENGTH_LENGTH +
                    ...
                    CHECKSUM_LENGTH + FRAME_TAIL_LENGTH;// 头部帧总长度
    ...
    private ByteBuf ByteArrayToByteBuf(byte[] array) {// 将字节数组转换为ByteBuf
        ByteBuf buffer = ByteBufAllocator.DEFAULT.buffer();// 创建一个ByteBuf
        buffer.writeBytes(array);// 将字节数组写入ByteBuf
        return buffer;// 返回ByteBuf
    }
}
